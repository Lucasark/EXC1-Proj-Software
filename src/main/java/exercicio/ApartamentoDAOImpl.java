...
public class ApartamentoDAOImpl implements ApartamentoDAO {
    ...
    public void altera(Apartamento umApartamento) throws AptNaoEncontradoException {
        EntityManager em = null;
        EntityTransaction tx = null;
        ...
        try {
            ...
            apartamento = em.find(Apartamento.class, umApartamento.getId(), LockModeType.PESSIMISTIC_WRITE);

            if (apartamento == null) {
                tx.rollback();
                throw new AptNaoEncontradoException("Produto não encontrado");
            }
            em.merge(umApartamento);
            tx.commit();
